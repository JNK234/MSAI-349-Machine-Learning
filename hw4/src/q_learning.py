...
class QLearning:
    ...
    def fit(self, env, steps=1000, num_bins=100):
        ...
        for _ in range(steps):
          ...
          if r < self.epsilon:
            ...
            best_actions = np.where(state_action_values[current_state]==max_value)[0]
          ...
          if not done:
             future_value = np.max(state_action_values[next_state])
             new_value = state_action_values[current_state][action] + self.alpha * (reward + self.gamma * future_value - state_action_values[current_state, action])
          else:
            new_value = state_action_values[current_state][action] + self.alpha * (reward - state_action_values[current_state, action])
            next_state, _ = env.reset()
            
          state_action_values[current_state, action] = new_value
        ...
        for bin_idx in range(num_bins):
          ...
          end_idx = min((bin_idx + 1) * bin_size, steps)
          if end_idx > start_idx:
            avg_rewards[bin_idx] = np.mean(all_rewards[start_idx:end_idx])
    ...
    def predict(self, env, state_action_values):
        ...
        while not (terminated or truncated):
            max_value = np.max(state_action_values[current_state])
            best_actions = [a for a, q in enumerate(state_action_values[current_state]) if q == max_value]
            action = rnd.choice(best_actions)
